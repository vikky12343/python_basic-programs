...
def insert():
    try:
        ...
        cg=int(cgpa.get())
        if crs=="":
            a=askyesno('Hold on!','Did you mean B.Tech ?')
            if a==True:
                crs='B.Tech'
        if drs=="":
            a=askyesno('Hold on!','Did you mean CSE ?')
            if a==True:
                drs='CSE'
        if cg <= 0:
            showerror('Error!','CGPA can not be < 0')
        
        cur.execute("insert into record values(?,?,?,?,?,?,?,?,?,?,?,?)",(eno.get(),fname.get(),lname.get(),dob.get(),father.get(),crs,drs,addr.get(),mobile.get(),email.get(),cgpa.get(),certi.get()))
